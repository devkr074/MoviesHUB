        ...
        <form onSubmit={handleSubmit} className="flex flex-col space-y-4 p-4 bg-gray-800 rounded-lg shadow-md">
            ...
            <div className="flex items-center space-x-2">
                <input
                    ...
                    name="otp"
                    placeholder="OTP"
                    className="flex-grow border border-2 border-white text-white bg-gray-700 p-2 rounded" value={formData.otp} onChange={handleChange} disabled={!otpSent} required={otpSent}
                />
                <button type="button" className={`px-4 py-2 rounded ${(formData.email && formData.username && formData.password && formData.confirmPassword && formData.password === formData.confirmPassword && !errors.email && !errors.username && !errors.password && !errors.confirmPassword) ? 'bg-blue-500 hover:bg-blue-600 text-white' : 'bg-gray-500 cursor-not-allowed text-gray-300'}`} onClick={handleGetOtp} disabled={!(formData.email && formData.username && formData.password && formData.confirmPassword && formData.password === formData.confirmPassword) || !!errors.email || !!errors.username || !!errors.password || !!errors.confirmPassword}>{otpSent ? 'Resend OTP' : 'Get OTP'}</button>
            </div>
            {errors.otp && <p className="text-red-500 text-sm">{errors.otp}</p>}
            <button type="submit" className={`bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded mt-4 ${!otpSent || !formData.otp ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={!otpSent || !formData.otp}>Signup</button>
        </form>
    );
}
