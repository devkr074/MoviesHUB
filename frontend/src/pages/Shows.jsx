        ...
        <>
            ...
            <div className="flex flex-wrap">
                {loading ?
                    ...
                    shows.map((s, index) => (
                        <Card key={index} type="shows" slug={s.show.ids.slug} category={category} image={`https://${s.show.images.fanart}` || null} title={s.show.title} year={s.show.year} rating={s.show.rating} watchers={s.watchers} waiting={s.list_count} rank={s.rank} delta={s.delta} release={s.first_aired} />
                    ))}
            </div>
            <Footer />
        </>
