...
function Shows() {
    ...
    useEffect(() => {
        document.title = `${tab[0].toUpperCase()}${tab.substring(1)} Shows - MoviesHUB`;
        getData(`https://api.trakt.tv/shows/${tab}?extended=full,images`).then(data => {
            setData(data);
        }).catch(err => {
            console.error(err);
        });
