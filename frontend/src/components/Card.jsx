        ...
        <div className={`h-full bg-cover bg-center bg-no-repeat ${props.category == "Home" && "rounded rounded-xl"} relative overflow-hidden`} style={{ backgroundImage: `url(${placeholder})` }}>
          ...
          <div style={{ backgroundImage: "linear-gradient(to bottom, rgba(0, 0, 0, 0) 0%, rgb(0, 0, 0) 100%)" }} className="w-full px-3 py-2 absolute bottom-0">
            {props.category == "Trending" &&
              <span className="h-6 bg-[#9F42C6] px-2 py-1 text-white text-xs rounded font-semibold">{props.watchers.toLocaleString('en-US')} Watchers</span>
            }

            {props.category == "Anticipated" &&
              <>
                <span className="h-6 bg-[#0082CE] px-2 py-1 text-white text-xs rounded font-semibold">{props.waiting.toLocaleString()} Anticipated</span>
              </>
            }
            {props.category == "Streaming" &&
              <div className="flex gap-1">
                ...
                {props.delta && <span className={`bg-black border ${props.delta > 0 ? "border-[#47AD45] text-[#47AD45]" : "border-[#FF0000] text-[#FF0000]"}  rounded-xl px-2 text-xs rounded font-semibold flex items-center gap-1`}>{props.delta > 0 ? <><i class="fa-solid fa-caret-up mt-[4px]"></i>+{props.delta}</> : <><i class="fa-solid fa-caret-down"></i>{props.delta}</>}</span>
                }</div>
            }
